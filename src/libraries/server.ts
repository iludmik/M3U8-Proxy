    ...
    return function (req, res) {
        ...
        if (!/^\/https?:/.test(req.url) && !isValidHostName(location.hostname)) {
            ...
            if (uri.pathname === "/m3u8-proxy") {
                ...
                return proxyM3U8(url ?? "", headers, res, req);
...
function getBaseUrl(req) {
    // For Vercel deployment, use the host header
    if (req.headers.host && req.headers.host.includes('vercel.app')) {
        return `https://${req.headers.host}`;
    }
    // For other deployments, use PUBLIC_URL if set
    if (process.env.PUBLIC_URL) {
        return process.env.PUBLIC_URL;
    }
    // Default to the configured web_server_url
 ...
 */
export async function proxyM3U8(url: string, headers: any, res: http.ServerResponse, req?: any) {
    const baseUrl = req ? getBaseUrl(req) : baseUrl;
    
    const axiosReq = await axios(url, {
        headers: headers,
    }).catch((err) => {
        res.writeHead(500);
        res.end(err.message);
    ...
    if (m3u8.includes("RESOLUTION=")) {
        ...
        for (const line of lines) {
            if (line.startsWith("#")) {
                if (line.startsWith("#EXT-X-KEY:")) {
                    ...
                    const url = `${baseUrl}${"/ts-proxy?url=" + encodeURIComponent(regex.exec(line)?.[0] ?? "") + "&headers=" + encodeURIComponent(JSON.stringify(headers))}`;
                    newLines.push(line.replace(regex, url));
                } else if (line.startsWith("#EXT-X-MEDIA:")) {
                    const regex = /https?:\/\/[^\""\s]+/g;
                    const url = `${baseUrl}${"/m3u8-proxy?url=" + encodeURIComponent(regex.exec(line)?.[0] ?? "") + "&headers=" + encodeURIComponent(JSON.stringify(headers))}`;
                    newLines.push(line.replace(regex, url));
                } else {
                    newLines.push(line);
                }
                ...
                newLines.push(`${baseUrl + "/m3u8-proxy?url=" + encodeURIComponent(uri.href) + "&headers=" + encodeURIComponent(JSON.stringify(headers))}`);
            }
        }
        ...
        for (const line of lines) {
            if (line.startsWith("#")) {
                ...
                newLines.push(`${baseUrl}${"/ts-proxy?url=" + encodeURIComponent(uri.href) + "&headers=" + encodeURIComponent(JSON.stringify(headers))}`);
            }
        }
